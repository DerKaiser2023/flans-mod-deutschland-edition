...
public class EntitySmokeShell extends EntityFX
{
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntitySmokeShell.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntitySmokeShell.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntitySmokeShell.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * colorIntensity, this.particleBlue * colorIntensity, 1.0f - this.particleAge * this.particleAge * this.particleAge * 8.1E-9f);
