...
public class EntityFMTracerRed extends EntityFX
{
    ...
    public EntityFMTracerRed(final World w, final double px, final double py, final double pz, final double mx, final double my, final double mz) {
        ...
        this.motionY = my;
        this.motionZ = mz;
        FlansMod.proxy.spawnParticle("flansmod.FMTracerRed", this.posX, this.posY, this.posZ, 0.0, 0.0, 0.0);
        EntityFMTracerRed.icon = new ResourceLocation("flansmod", "particle/FMTracerRed.png");
    }
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntityFMTracerRed.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntityFMTracerRed.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntityFMTracerRed.interpPosZ);
