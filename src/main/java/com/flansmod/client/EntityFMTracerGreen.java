...
public class EntityFMTracerGreen extends EntityFX
{
    ...
    public EntityFMTracerGreen(final World w, final double px, final double py, final double pz, final double mx, final double my, final double mz) {
        ...
        this.motionY = my;
        this.motionZ = mz;
        FlansMod.proxy.spawnParticle("flansmod.FMTracerGreen", this.posX, this.posY, this.posZ, 0.0, 0.0, 0.0);
        EntityFMTracerGreen.icon = new ResourceLocation("flansmod", "particle/FMTracerGreen.png");
    }
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float scale = 0.6f - this.particleAge * 0.1f;
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntityFMTracerGreen.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntityFMTracerGreen.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntityFMTracerGreen.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * (colorIntensity - this.particleAge * 0.2f), this.particleBlue * (colorIntensity - this.particleAge * 0.2f), 1.0f - this.particleAge * 0.1f);
