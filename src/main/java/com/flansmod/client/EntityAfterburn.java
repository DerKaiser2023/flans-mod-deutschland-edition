...
public class EntityAfterburn extends EntityFX
{
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float scale = 0.8f - this.particleAge * 0.13f;
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntityAfterburn.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntityAfterburn.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntityAfterburn.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * (colorIntensity - this.particleAge * 0.2f), this.particleGreen * (colorIntensity - this.particleAge * 0.2f), this.particleBlue * colorIntensity, 1.0f - this.particleAge * 0.1f);
