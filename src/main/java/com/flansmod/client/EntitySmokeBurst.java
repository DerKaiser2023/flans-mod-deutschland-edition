...
public class EntitySmokeBurst extends EntityFX
{
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        FMLClientHandler.instance().getClient().renderEngine.bindTexture(EntitySmokeBurst.icon);
        final float scale = 5.0f;
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntitySmokeBurst.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntitySmokeBurst.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntitySmokeBurst.interpPosZ);
