...
public class EntitySmallSmoke extends EntityFX
{
    ...
    public EntitySmallSmoke(final World w, final double px, final double py, final double pz, final double mx, final double my, final double mz) {
        super(w, px, py, pz, mx, my, mz);
        this.particleMaxAge = 100;
        this.particleGravity = 1.0f;
        this.motionX = mx * 1.0E-5;
        this.motionY = my * 1.0E-5;
        this.motionZ = mz * 0.1;
        this.particleScale = 0.0f;
        this.disTime = 10;
        EntitySmallSmoke.icon = new ResourceLocation("flansmod", "particle/FMSmoke.png");
    }
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float scale = 2.5f + this.particleAge * 0.01f;
        final float xPos = (float)(this.prevPosX + 1.0E-4 * (this.posX - this.prevPosX) * par2 - EntitySmallSmoke.interpPosX);
        final float yPos = (float)(this.prevPosY + 1.0E-5 * (this.posY - this.prevPosY) * par2 - EntitySmallSmoke.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntitySmallSmoke.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * colorIntensity, this.particleBlue * colorIntensity, 1.0f - this.particleAge * this.particleAge * this.particleAge * 3.21E-7f);
