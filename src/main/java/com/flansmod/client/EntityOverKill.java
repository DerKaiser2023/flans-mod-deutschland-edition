...
public class EntityOverKill extends EntityFX
{
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float scale = 0.1f + this.particleAge * 0.4f;
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntityOverKill.interpPosX);
        final float yPos = (float)(this.prevPosY + 5.0 + (this.posY - this.prevPosY) * par2 - EntityOverKill.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntityOverKill.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * (colorIntensity - this.particleAge * 0.02f), this.particleGreen * (colorIntensity - this.particleAge * 0.05f), this.particleBlue * (colorIntensity - this.particleAge * 0.05f), 1.0f - this.particleAge * this.particleAge * this.particleAge * 0.0081f);
