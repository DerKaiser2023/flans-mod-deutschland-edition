...
public class EntityRocketexhaust extends EntityFX
{
    ...
    public EntityRocketexhaust(final World w, final double px, final double py, final double pz, final double mx, final double my, final double mz) {
        ...
        this.motionY = my;
        this.motionZ = mz;
        EntityRocketexhaust.icon = new ResourceLocation("flansmod", "particle/FMSmoke.png");
    }
    ...
    public void renderParticle(final Tessellator par1Tessellator, final float par2, final float par3, final float par4, final float par5, final float par6, final float par7) {
        ...
        final float scale = 10.0f + this.particleAge * 0.05f;
        final float xPos = (float)(this.prevPosX + (this.posX - this.prevPosX) * par2 - EntityRocketexhaust.interpPosX);
        final float yPos = (float)(this.prevPosY + (this.posY - this.prevPosY) * par2 - EntityRocketexhaust.interpPosY);
        final float zPos = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * par2 - EntityRocketexhaust.interpPosZ);
        final float colorIntensity = 1.0f;
        par1Tessellator.setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * colorIntensity, this.particleBlue * colorIntensity, 1.0f - this.particleAge * 0.1f);
